...
void TestHashLookup(void)
{
	...
	{
		FAILTEST(TestHashLookup, "Failed to lookup recently created element!");
	}
	
	if (GerbilNew1 != 777 || GerbilNew2 != 444)
	{
		FAILTEST(TestHashLookup, "Retrieved values from hash do not match!");
	}
	...
	{
		FAILTEST(TestHashLookup, "Node not deleted properly!");
	}
	
	if (!CPSL_Hash_Get(Hash, "Three", NULL))
	{
		FAILTEST(TestHashLookup, "Destruction of other nodes resulted in destruction of one we wanted to keep!");
	}
